...
from data_preprocess.utils import to_graph, to_edges, using_split2
from tqdm import tqdm

sys.path.append('./data_preprocess/common/genia-tagger-py/')
from geniatagger import GENIATagger

pwd = '/'.join(os.path.realpath(__file__).split('/')[:-1])
genia_splitter = os.path.join("/common", "geniass")
genia_tagger = GENIATagger(
    os.path.join("./data_preprocess/common", "genia-tagger-py", "geniatagger-3.0.2", "geniatagger"))
...
def generate_pairs(uents, type1, type2, true_rels):
    ...
    for c in combs:
            ...
            if list(set(tr.arg1).intersection(set(c[0]))) and list(set(tr.arg2).intersection(set(c[1]))):
                for t1, t2 in zip(type1, type2):
                    if uents[c[0]][0].type == t1 and uents[c[1]][0].type == t2:
                        pairs[(c[0], c[1])] = \
                            PairStruct(tr.pmid, tr.type, c[0], c[1], 'L2R', cross_res, (a1, a2))
                        found_rels += 1
            # BA existing relation
                ...
                for t1, t2 in zip(type1, type2):
                    if uents[c[1]][0].type == t1 and uents[c[0]][0].type == t2:
                        pairs[(c[1], c[0])] = \
                            PairStruct(tr.pmid, tr.type, c[1], c[0], 'R2L', cross_res, (a2, a1))
                        found_rels += 1
            # relation not found
            else:
                not_found_rels += 1
        ...
        if not_found_rels == total_rels:
            for t1, t2 in zip(type1, type2):
                if uents[c[0]][0].type == t1 and uents[c[1]][0].type == t2:
                    pairs[(c[0], c[1])] = PairStruct(a1.pmid, 'NR', c[0], c[1], 'L2R', cross_res, (a1, a2))
                    unk += 1
                elif uents[c[1]][0].type == t1 and uents[c[0]][0].type == t2:
                    pairs[(c[1], c[0])] = PairStruct(a1.pmid, 'NR', c[1], c[0], 'R2L', cross_res, (a2, a1))
                    unk += 1
...
def adjust_offsets(old_sents, new_sents, old_entities, show=False):
    ...
    for e in old_entities:
        ...
        if (start, end) not in terms:
            terms[(start, end)] = [[start, end, e.type, e.name, e.pmid, e.kb_id, 0, 0]]
        else:
            terms[(start, end)].append([start, end, e.type, e.name, e.pmid, e.kb_id, 0, 0])
    ...
    # sents_list = [s.split(' ') for s in new_sents]
    ent_sequences = []
