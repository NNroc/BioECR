            ...
            if l_i <= max_len:
                ...
                while flag:
                    ...
                    if pad == 0:
                        ...
                        ids_with_pad = torch.cat(
                            [start_token, input_ids[i, left: right], end_token, pad_token.repeat(pad)], dim=-1)
        ...
        for (n_s, l_i) in zip(num_seg, seq_len):
            if n_s == 1:
                ...
                for j in range(n_s):
                    ...
                    segment_attention = F.pad(segment_attention,
                                              (bias, c - (bias + right - left), bias, c - (bias + right - left)))
