...
from .loss_func import ATLoss, SCELoss, categorical_crossentropy
from .config import *
...
class RelationModel(BaseModel):
    def __init__(self, hidden_dim: int, num_class: int, block_dim: int = 64, sample_rate: float = 0.0,
                 beta: float = 1.0, max_pred: int = -1, aggr_func: Callable = mean_vote, dataset: str = ""):
        super().__init__(hidden_dim, hidden_dim, block_dim, num_class, sample_rate, ATLoss(beta))
        self.max_pred = max_pred
        self.aggr_func = aggr_func
        self.dataset = dataset

    def inference(self, head_embed: torch.Tensor = None, tail_embed: torch.Tensor = None, span_len=None, batch_hts=None,
                  batch_clusters=None, logits: torch.Tensor = None):
        """
        ...
        for i in range(num_ent):
            for j in range(num_ent):
                ...
                for r in preds:
                    if r == 0:
                        continue
                    if (clusters_type[i], clusters_type[j], r) not in pairs2rel and (
                            clusters_type[j], clusters_type[i], r) not in pairs2rel:
                        continue
